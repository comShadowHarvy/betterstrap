...
# We need to skip the immediate execution parts if any. 
# Looking at the file, it sets constants and has a trap, but doesn't run main logic immediately at top level except for the trap and set constants.
...
run_test() {
    ...
    # Run detection
    # We need to source inside a subshell to avoid polluting state, 
    # but we need to extract the results.
    (
