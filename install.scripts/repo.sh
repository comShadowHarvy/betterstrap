...
ensure_option() {
    local desired_line="$1"; local keyword="$2"
...
set_insecure_siglevel() {
    ...
    log warn "This means you could install malicious packages from that repo without any warning. This is a ${C_BOLD}significant security risk${C_RESET}${C_YELLOW} and should only be used for a specific, trusted repository."
    ...
    local repos; repos=($(grep -E "^\s*\[.+\]" /etc/pacman.conf | sed -E 's/\[|\]|#//g' | awk '{$1=$1};1'))
...
apply_pacman_tweaks() {
    ...
    if [[ "$answer" =~ ^[Yy]$ ]]; then
        ...
        ensure_option "#UseSyslog" "UseSyslog"; ensure_option "Color" "Color"; ensure_option "ILoveCandy" "ILoveCandy"
        ensure_option "#NoProgressBar" "NoProgressBar"; ensure_option "CheckSpace" "CheckSpace"; ensure_option "VerbosePkgLists" "VerbosePkgLists"
        ensure_option "DisableDownloadTimeout" "DisableDownloadTimeout"; ensure_option "ParallelDownloads = 10" "ParallelDownloads"
        ensure_option "DownloadUser = alpm" "DownloadUser"; ensure_option "#DisableSandbox" "DisableSandbox"
        log success "Pacman options have been configured."
    else
        log info "Skipping general Pacman configuration tweaks."
    fi

    # Always ask about the dangerous option separately
    set_insecure_siglevel
}

# --- Repository Setup Functions ---
# (These functions remain the same as the previous version)
setup_cachyos() { log header "Setting up CachyOS Repository"; cd "$TMP_DIR" || return 1; curl -O 'https://mirror.cachyos.org/cachyos-repo.tar.zst' || { handle_error "Failed to download cachyos-repo.tar.zst"; return 1; }; tar -xvf cachyos-repo.tar.zst || { handle_error "Failed to extract cachyos-repo.tar.zst"; return 1; }; cd cachyos-repo || return 1; sudo ./cachyos-repo.sh || { handle_error "CachyOS setup script failed."; return 1; }; log success "CachyOS repository and keyring have been added."; log warn "${C_BOLD}ACTION REQUIRED:${C_RESET} ${C_YELLOW}For CachyOS packages to have priority, you ${C_BOLD}MUST${C_RESET}${C_YELLOW} manually edit '/etc/pacman.conf' and move the CachyOS repo blocks ${C_BOLD}BEFORE${C_RESET}${C_YELLOW} the standard repos."; }
add_standard_arch_repos() { log header "Adding Standard Arch Linux Repositories"; log warn "This is for systems like CachyOS that replace the default repos."; local standard_repos_block; standard_repos_block=$(cat <<'EOF'
[core]
...
Include = /etc/pacman.d/mirrorlist
EOF
); echo "$standard_repos_block" | sudo tee -a /etc/pacman.conf > /dev/null; log success "Standard Arch repositories added. You may need to edit /etc/pacman.d/mirrorlist."; }
